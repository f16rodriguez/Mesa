...
function renderPhone(){hideWorld();const s=view.seat,mia=s%2,compa=(s+2)%4,jugando=view.phase==='playing';
 ...
 else{const r=view.result,gano=r?.team===mia,parejo=r?.team==null;cuerpo=`<div class="phone-wait fin ${parejo?'':gano?'gano':'perdio'}"><div class="eyebrow">${r?.zapato?t('zapato'):r?.type==='capicua'?t('capicua'):r?.type==='tranque'?t('tranque'):t('domino')}</div><h1>${parejo?t('parejo'):gano?t('ganamos'):t('perdimos')}</h1>${!parejo&&gano?`<div class="points">${t('puntos',{n:r.points})}</div>`:''}<p>${r?.pips?.[s]?t('tusPuntos',{n:r.pips[s]}):''}${view.opener!=null&&view.phase==='handEnd'?'<br>'+t('sale',{nombre:esc(nombreDe(view.opener))}):''}</p>${view.phase==='seriesEnd'?cuentaCard():''}${view.phase==='handEnd'?listoTel():`<button class="g-button primary full" data-action="newSeries" data-tras-espera ${faltaFin()>0?'disabled':''}>${t('otraSerie')}</button>${view.isVip?button(t('cambiarParejas'),'lobby','full'):''}${listaCard('fin_serie')}`}</div>`;
...
function creditos(){openModal('',`<div class="acerca"><img class="logo-estudio" src="/marca/three-thirteen-logo-light-on-dark.svg" alt="Three Thirteen Studios"><p class="lugar">${t('mesaDeEstudio')}</p><p class="autor">${t('creadoPor')} Trey Rodriguez</p><p class="derechos">© 2026 Three Thirteen Studios · <a href="https://threethirteenstudios.pages.dev" target="_blank" rel="noopener">threethirteenstudios.pages.dev ↗</a></p><p class="fuentes">${t('fuentesEstudio')}</p><div class="usa"><h3>${t('mesaUsa')}</h3><p>three.js (MIT) · ${t('personajes')}: Quaternius (CC0)</p></div>${button(t('listo2'),'close','full')}</div>`,'estudio');}
...
document.addEventListener('click',async e=>{
 ...
 else if(a==='lista')listaModal(b.dataset.origen==='fin_practica'?'fin_practica':'fin_serie');
...
document.addEventListener('submit',async e=>{
 ...
 else if(f.id==='lista-form'){const b=f.querySelector('button');b.disabled=true;$('#lista-error').textContent='';const origen=f.dataset.origen;
  try{await api('lista',{email:String(data.get('email')||'').trim(),lang:idioma(),source:origen});almacen.set('mesa-lista','si');telemetria.evento('lista',{paso:'enviada',origen});modal.close();toast(t('listaListo'));if(page==='room'&&view)renderRoom();}
