...
export const sonidos={ctx:null,bus:null,activo:true,ruido:null,
 ...
 /** El corte pintado: un barrido de aire, como una página que entra de golpe. */
 corte(){if(!this.activo||!this.abrir())return;const c=this.ctx,t=c.currentTime+.01,n=c.createBufferSource(),bp=c.createBiquadFilter(),g=c.createGain();n.buffer=this.ruido;n.loop=true;
  bp.type='bandpass';bp.Q.value=1.2;bp.frequency.setValueAtTime(320,t);bp.frequency.exponentialRampToValueAtTime(3200,t+.3);
  g.gain.setValueAtTime(.0008,t);g.gain.exponentialRampToValueAtTime(.22,t+.1);g.gain.exponentialRampToValueAtTime(.0008,t+.42);n.connect(bp).connect(g).connect(this.bus);n.start(t);n.stop(t+.45);},
