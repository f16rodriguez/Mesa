...
export async function createWorld(container,{onProgress=()=>{},cast=MESA_CLASICA}={}){
 ...
 window.mesaGiros=()=>characters.map(c=>{if(!c?.head||!c.front)return null;const h=c.head.getWorldPosition(v3()),f=c.front.getWorldPosition(v3()).sub(h),b=v3(0,0,1).applyQuaternion(c.holder.getWorldQuaternion(new THREE.Quaternion()));return +Math.atan2(Math.sin(Math.atan2(f.x,f.z)-Math.atan2(b.x,b.z)),Math.cos(Math.atan2(f.x,f.z)-Math.atan2(b.x,b.z))).toFixed(2);});   // giro de la cara respecto al cuerpo, en radianes
